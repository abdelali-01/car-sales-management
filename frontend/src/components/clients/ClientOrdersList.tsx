        ...
        <div className="rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800/50 p-6 shadow-sm flex flex-col">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-6 flex items-center gap-2">
                ...
                <span className="ml-auto text-xs font-normal text-gray-500 bg-gray-100 dark:bg-gray-800 px-2 py-1 rounded-full">
                    {orders?.length || 0} {orders?.length === 1 ? t('clients.orders.count_one') : t('clients.orders.count_other')}
                </span>
            </h3>
            ...
            {!orders || orders.length === 0 ? (
                <div className="flex flex-col items-center justify-center flex-1 py-10 text-center">
                    ...
                    <p className="text-gray-500 dark:text-gray-400">{t('clients.orders.noOrders')}</p>
                </div>
            ) : (
                            ...
                            <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-sm text-gray-500 dark:text-gray-400">
                                            {order.offer
                                                ...
                                                : order.orderedCar
                                                    ...
                                                    : t('clients.orders.customRequest')}
                                        </p>
                                    </div>
                                ...
                                <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
                                    <div className="flex flex-col">
                                        <span className="text-xs text-gray-500 dark:text-gray-400">{t('clients.orders.orderDate')}</span>
                                        <span className="font-medium text-gray-900 dark:text-white">
                                            {format(new Date(order.createdAt), 'MMM dd, yyyy')}
                                        </span>
                                    </div>
                                    <div className="flex flex-col text-right">
                                        <span className="text-xs text-gray-500 dark:text-gray-400">{t('clients.orders.agreedPrice')}</span>
